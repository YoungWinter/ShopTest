...
public class OrderDao {
	...
	public void updateOrder(Order order) throws SQLException {
		...
		qr.update(sql, order.getAddress(), order.getName(),
				order.getTelephone(), order.getState(), order.getOid());
	}
	...
	// 通过订单项对象查询订单项下的产品对象
	public Product findProductByItemid(OrderItem orderItem)
			throws SQLException {
		QueryRunner qr = new QueryRunner(DataSourceUtils.getDataSource());
	...
	public List<Map<String, Object>> findOrderInfoByOid(String oid)
			throws SQLException {
		QueryRunner qr = new QueryRunner(DataSourceUtils.getDataSource());
		String sql = "select o.count,o.subtotal,o.pid from orderitem o where oid=?";
		List<Map<String, Object>> list = qr.query(sql, new MapListHandler(),
				oid);
		for (Map<String, Object> map : list) {
			String key = (String) map.get("pid");
			sql = "select p.pimage,p.pname,p.shop_price from product p where pid=?";
			Map<String, Object> product = qr.query(sql, new MapHandler(), key);
			map.put("pimage", product.get("pimage"));
			map.put("pname", product.get("pname"));
			map.put("shop_price", product.get("shop_price"));
		}
		return list;
	}
